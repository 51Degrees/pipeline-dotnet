...
#pragma warning restore CA1721 // Property names should not match get methods
        ...
        protected override void ProcessEngine(IFlowData data, T aspectData) {
            ...
            {
                // The upstream CloudRequestEngine produced no JSON for this
                // request. The most likely cause is that it threw an
                // exception, which Pipeline.Process records on
                // FlowData.Errors against that engine instance. Use that
                // existing FlowData signal to detect the failure and
                // stamp a marker on this engine's aspect data so that
                // subsequent property accesses (via
                // AspectDataBase.GetAs<T>) can report
                // MissingPropertyReason.CloudRequestFailed instead of
                // falling through to misleading reasons such as
                // DataFileUpgradeRequired. Treating FlowData.Errors as the
                // source of truth means the CloudRequestEngine itself
                // needs no special handling — its exception flows through
                // the standard Pipeline error path.
                if (CloudRequestFailedForCurrentRequest(data) &&
                    aspectData is AspectDataBase aspectDataBase)
                {
                    // TODO LA - Write error to FlowData.Errors instead of using a marker method on AspectDataBase.
                    //aspectDataBase.MarkCloudRequestFailed();

                    data.Errors.Add(new FlowError(this, "Cloud request failed. See previous errors for details.")); // TODO LA - Refactor
                }
        ...
        /// <summary>
        /// Inspect <see cref="IFlowData.Errors"/> to determine whether the
        /// <see cref="CloudRequestEngine"/> that feeds this engine recorded
        /// an error during processing of the current request.
        /// </summary>
        /// <param name="data">
        /// The <see cref="IFlowData"/> instance for the current request.
        /// </param>
        /// <returns>
        /// True if <see cref="IFlowData.Errors"/> contains an entry whose
        /// <see cref="IFlowError.FlowElement"/> is the upstream
        /// <see cref="CloudRequestEngine"/>.
        /// </returns>
        private bool CloudRequestFailedForCurrentRequest(IFlowData data)
        {
            if (data?.Errors == null || data.Errors.Count == 0)
            {
                return false;
            }
            var requestEngine = RequestEngine.GetInstance();
            foreach (var error in data.Errors)
            {
                if (error != null &&
                    ReferenceEquals(error.FlowElement, requestEngine))
                {
                    return true;
                }
            }
