...
namespace FiftyOne.Pipeline.Core.Tests.Data
{
    ...
    {
        private EvidenceKeyFilterAggregator _filter;

        [TestMethod]
        /// <summary>
        /// Check that the Include method works as expected for an
        /// aggregated filter that contains two white list filters.
        /// </summary>
        public void EvidenceKeyFilterAggregator_Include_Whitelists()
        {
            var filter1 = new EvidenceKeyFilterWhitelist(new List<string>() { "key1", "key2" });
            var filter2 = new EvidenceKeyFilterWhitelist(new List<string>() { "key3", "key4" });
            ...
            Assert.IsFalse(_filter.Include("key5"));
        }
        ...
        public void EvidenceKeyFilterAggregator_Include_Aggregated()
        {
            ...
            var filter3 = new EvidenceKeyFilterWhitelist(new List<string>() { "key5", "key6" });
            _filter = new EvidenceKeyFilterAggregator();
        ...
        {
            var filter1 = new EvidenceKeyFilterWhitelist(new List<string>() { "key1" }, 
                StringComparer.OrdinalIgnoreCase);
            var filter2 = new EvidenceKeyFilterWhitelist(new List<string>() { "key2" }, 
