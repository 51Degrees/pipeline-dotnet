...
namespace FiftyOne.Pipeline.CloudRequestEngine.Tests.Data
{
    [TestClass]
    public class CloudAspectDataBaseTests
    {
        private class TestCloudData : CloudAspectDataBase
        {
            public TestCloudData(
        ...
        private TestCloudData _data;
        private TestLogger<AspectDataBase> _logger;
        ...
        public void Init()
        {
            _logger = new TestLogger<AspectDataBase>();
        ...
        /// in <see cref="CloudAspectDataBase"/> overrides the missing-property
        /// service result and throws with reason
        /// <see cref="MissingPropertyReason.CloudRequestFailed"/>.
        /// </summary>
        [TestMethod]
        public void GetMissing_CloudRequestFailed_WhenErrorAndEmpty()
        {
            ...
            StringAssert.Contains(ex.Message, "upstream timeout");
        }
        ...
        /// </summary>
        [TestMethod]
        public void GetMissing_DelegatesToService_WhenDataNotEmpty()
        {
            var failingElement = new Mock<IFlowElement>();
            var flowData = new Mock<IFlowData>();
            flowData.SetupGet(d => d.Errors).Returns(new List<IFlowError>
            ...
            Assert.AreEqual(MissingPropertyReason.Unknown, ex.Reason);
        ...
        /// </summary>
        [TestMethod]
        public void GetMissing_DelegatesToService_WhenNoErrors()
        {
            var flowData = new Mock<IFlowData>();
            flowData.SetupGet(d => d.Errors).Returns(new List<IFlowError>());
            _data.SetFlowData(flowData.Object);
        ...
        /// <summary>
        /// If <see cref="CloudAspectDataBase.SetFlowData"/> was never called
        /// (cloud aspect data created outside the standard cloud-engine
        /// pipeline), the heuristic must short-circuit cleanly rather than
        /// NRE -- delegating to the existing service path.
        /// </summary>
        [TestMethod]
        public void GetMissing_DelegatesToService_WhenFlowDataUnset()
        {
            var ex = Assert.ThrowsExactly<PropertyMissingException>(
                () => { var _ = _data["testproperty"]; });
        ...
        /// </summary>
        [TestMethod]
        public void CloudRequestFailed_DescriptionIdentifiesUpstreamElement()
        {
            var failingElement = new Mock<IFlowElement>();
            var flowData = new Mock<IFlowData>();
            flowData.SetupGet(d => d.Errors).Returns(new List<IFlowError>
            ...
            StringAssert.Contains(ex.Message, failingElement.Object.GetType().Name);
            StringAssert.Contains(ex.Message, "network timeout");
        }
    }
}
