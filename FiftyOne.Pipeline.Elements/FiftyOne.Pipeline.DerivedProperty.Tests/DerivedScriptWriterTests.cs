...
/// test here compares against the output that reference printed, which the
/// submodule ships in canonical/ beside the scripts themselves.
/// </summary>
[TestClass]
public class DerivedScriptWriterTests
{
    ...
    /// <summary>
    /// The cross language test. Both the script and the text the
    /// JavaScript reference printed for it are read from the
    /// derived-properties submodule, the first from scripts/ and the
    /// second from canonical/, and the text printed here is compared with
    /// the second.
    ///
    /// Both come from the submodule on purpose. A copy of the printed form
    /// kept in this repository is stale from the moment a script changes,
    /// so the automatic submodule update could never carry one: it moved
    /// the pointer, the copy stayed behind, and the run failed on the copy
    /// rather than on anything that was wrong. Read from the submodule,
    /// the script and the print of it arrive in the same commit.
    ///
    ...
    {
        var path = FindInSubmodule("scripts", "HumanConfidence.yaml");
        if (path == null)
        {
        ...
        var reference = FindInSubmodule("canonical", "HumanConfidence.json");
        Assert.IsNotNull(
            reference,
            "The submodule is checked out but ships no printed form for " +
            "HumanConfidence in canonical/, so the two languages were not " +
            "compared. The folder is generated by the tools of the " +
            "derived-properties repository and committed there.");

        // The submodule ships the printed form as a text file, so it ends
        // in a line feed where the writer's output does not.
        Assert.AreEqual(
            Normalise(File.ReadAllText(reference)).TrimEnd('\n'),
    ...
    /// <summary>
    /// Finds a file in a folder of the derived-properties submodule by
    /// walking up from the folder the tests run in until the checked out
    /// repository is found. Returns null where the submodule is not
    /// checked out, or where it is but does not hold the file.
    /// </summary>
    private static string FindInSubmodule(string subfolder, string name)
    {
