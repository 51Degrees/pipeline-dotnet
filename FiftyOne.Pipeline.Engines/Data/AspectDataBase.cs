...
namespace FiftyOne.Pipeline.Engines.Data
{
    ...
    public abstract class AspectDataBase : ElementDataBase, IAspectData
    {
        ...
        /// <summary>
        /// The key under which the cloud-request-failed marker is stored in
        /// the underlying <see cref="DataBase"/> dictionary. Exposed so that
        /// callers can introspect the flag via <see cref="DataBase.TryGet"/>
        /// if they need to, but the recommended access is via the
...
#pragma warning restore CA1707 // Identifiers should not contain underscores
        ...
        /// <summary>
        /// True if the cloud request that should have populated this aspect
        /// data failed for the request that produced this instance.
        /// <para>
        /// For <c>CloudRequestData</c> the flag is set by the
        /// <c>CloudRequestEngine</c> itself when the HTTP call throws.
        /// For downstream cloud aspect data (e.g. <c>DeviceDataCloud</c>)
        /// the flag is propagated by <c>CloudAspectEngineBase.ProcessEngine</c>
        /// when it observes that the upstream <c>CloudRequestData</c> failed.
        /// </para>
        /// <para>
        /// When this flag is set, <see cref="GetAs{T}"/> reports missing
        /// properties as <see cref="MissingPropertyReason.CloudRequestFailed"/>
        /// rather than falling through to the
        /// <see cref="IMissingPropertyService"/> heuristics, which cannot
        /// distinguish a transient request failure from a configuration
        /// issue.
        /// </para>
        /// </summary>
        public bool CloudRequestFailed
        {
            get
            {
                ...
                // than the base[key] indexer. The indexer dispatches through
                // the overridden GetAs<T> below, which itself reads this
                // property — that would cause unbounded recursion.
        ...
        /// <summary>
        /// Mark this aspect data as having been produced for a request whose
        /// upstream cloud call failed.
        /// <para>
        /// Called by <c>CloudRequestEngine</c> in its exception path, and
        /// propagated by <c>CloudAspectEngineBase</c> from
        /// <c>CloudRequestData</c> to downstream cloud aspect data. Setting
        /// this flag causes subsequent property accesses through
        /// <see cref="GetAs{T}"/> on this instance to throw
        /// <see cref="PropertyMissingException"/> with
        /// <see cref="MissingPropertyReason.CloudRequestFailed"/> instead of
        /// the misleading reasons that would otherwise be produced by the
        /// missing-property service (which has no per-request context).
        /// </para>
        /// </summary>
        ...
        protected override T GetAs<T>(string key)
        {
            ...
            if (gotProperty == false)
            {
                ...
                {
                    // Pass `this` to the service so it can detect a
                    // cloud-request failure recorded on this aspect data
                    // (via MarkCloudRequestFailed) and report
                    // MissingPropertyReason.CloudRequestFailed. Without the
                    // aspect data, the service has no per-request context
                    // and would fall through to a misleading reason such
                    // as DataFileUpgradeRequired.
        ...
        /// Warning level would create noise that obscures the underlying
        /// <c>CloudRequestException</c> (which the pipeline has already
        /// recorded against <c>FlowData.Errors</c>). So
        /// <see cref="MissingPropertyReason.CloudRequestFailed"/> is
        /// logged at Information and every other reason at Warning.
        /// </para>
        /// </remarks>
        /// <param name="key">The property name that was requested.</param>
        /// <param name="reason">The reason reported by the service.</param>
